...
class QLearner:
    ...
    def train_reg(self, ori_batch: EpisodeBatch, batch: EpisodeBatch, t_env: int, episode_num: int):
        # Get the relevant quantities
        rewards = batch["reward"][:, :-1]
        actions = batch["actions"][:, :-1]
        terminated = batch["terminated"][:, :-1].float()
        ...
        margins = []
        self.hidden_state = self.mac.init_hidden(batch.batch_size)
        self.ori_hidden_state = self.mac.init_hidden(ori_batch.batch_size)
        for t in range(batch.max_seq_length):
            agent_outs, hidden_state_ = self.mac.forward(batch, t=t, hidden_states=self.hidden_state)
            ori_agent_outs, ori_hidden_state_ = self.mac.forward(ori_batch, t=t, hidden_states=self.ori_hidden_state)
            # -------
        ...
        self.target_hidden_state = self.target_mac.init_hidden(batch.batch_size)
        for t in range(batch.max_seq_length):
            target_agent_outs, target_hidden_state_ = self.target_mac.forward(batch, t=t,
                                                                              hidden_states=self.target_hidden_state)
            self.target_hidden_state = target_hidden_state_
            target_mac_out.append(target_agent_outs)
        ...
        if self.args.attack_method == "adv_reg":
            loss = ((masked_td_error ** 2).sum() + reg_loss * self.args.kappa) / mask.sum()
        else:
            loss = (masked_td_error ** 2).sum() / mask.sum()
