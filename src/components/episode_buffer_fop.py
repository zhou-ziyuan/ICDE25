...
class ReplayBuffer_FOP(EpisodeBatch):

    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu", out_device=None):
        super(ReplayBuffer_FOP, self).__init__(scheme, groups, buffer_size, max_seq_length, preprocess=preprocess, device=device, out_device=out_device)
    ...
    def sample_index(self,batch_size,ep_ids):
        assert self.can_sample(batch_size)
        return self[ep_ids]
...
class CompressibleReplayBuffer(CompressibleEpisodeBatch, ReplayBuffer_FOP):
