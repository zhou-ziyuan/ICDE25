...
class CQMixMAC(BasicMAC):
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False, past_actions=None, critic=None,
        ...
        if self.args.agent in ["naf", "mlp", "rnn"]:
            # print(hidden_states)
            # aaa
            chosen_actions_x = self.forward(ep_batch[bs],
                                          t_ep,
                                          hidden_states=hidden_states[bs],
                                          test_mode=test_mode,
                                          select_actions=True) # just to make sure detach
            # print(chosen_actions_x)

            chosen_actions = chosen_actions_x["actions"]
            hidden_states_ = chosen_actions_x["hidden_state"]
            # print(chosen_actions.size())  # 打印大小以确认

            # # 检查 ep_batch[bs].batch_size, self.n_agents 和 self.args.n_actions 的值
            # print(ep_batch[bs].batch_size, self.n_agents, self.args.n_actions)
            chosen_actions = chosen_actions.view(ep_batch[bs].batch_size, self.n_agents, self.args.obs_shape).detach()
            # print(ep_batch)
            pass
            ...
            hidden_states = hidden_states[bs].repeat(N, 1, 1)
        ...
        return chosen_actions, hidden_states_
    ...
    def forward(self, ep_batch, t, actions=None, hidden_states=None, select_actions=False, test_mode=False):
        ...
        # print(agent_inputs.size())
        # aaa
        # print(11111111111111, agent_inputs)
        # print(22222222222222, hidden_states)

        ret = self.agent(agent_inputs, hidden_states, actions=actions)
        # print(ret)
        if select_actions:
            hidden_states_ = ret["hidden_state"]
            return ret
        # hidden_states_ = ret["hidden_state"]
        # return ret
        agent_outs = ret["Q"]
        ...
        if self.agent_output_type_mer == "pi_logits":
            agent_outs = th.nn.functional.softmax(agent_outs, dim=-1)
            if not test_mode:
                agent_outs = ((1 - self.action_selector.epsilon) * agent_outs
                               + th.ones_like(agent_outs) * self.action_selector.epsilon/agent_outs.size(-1))
        return agent_outs.view(ep_batch.batch_size, self.n_agents, -1), actions, hidden_states_

    def soft(self, agent_inputs, t, actions=None, test_mode=False, hidden_states=None):
        # agent_inputs = self._build_inputs(ep_batch, t)
        ret = self.agent(agent_inputs, hidden_states, actions=actions)
        ...
        # print(ret["logits"])
        return ret["logits"]
    def ac(self, agent_inputs, t, actions=None, test_mode=False, hidden_states=None):
        # agent_inputs = self._build_inputs(ep_batch, t)
        ret = self.agent(agent_inputs, hidden_states, actions=actions)
    ...
    def cem_sampling(self, ep_batch, t, bs, critic=None, hidden_states = None):
        ...
        hidden_states = hidden_states.reshape(-1, self.n_agents, self.args.rnn_hidden_dim)[bs].repeat(N, 1, 1, 1)
